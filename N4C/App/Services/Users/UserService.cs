...
namespace N4C.App.Services.Users
{
    ...
    {
        public UserService(IDb db, HttpService httpService, ILogger<Service> logger) : base(db, httpService, logger)
        {
        ...
        public async Task<Result<UserResponse>> GetItem(string userName, string password, CancellationToken cancellationToken = default)
        {
            ...
            {
                Logger.LogError($"UserServiceException: {GetType().Name}.GetItem(UserName = {userName}): {exception.Message}");
