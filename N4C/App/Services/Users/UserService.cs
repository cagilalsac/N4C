...
namespace N4C.App.Services.Users
{
    public class UserService : Service<User, UserRequest, UserResponse>
    {
        public UserService(IDb db, HttpService httpService, FileService fileService, LogService logService) : base(db, httpService, fileService, logService)
        {
            ...
            {
                c.Response
                    .Property(d => d.ActiveS, s => s.Active.ToHtml(c.TrueHtml, c.FalseHtml))
        ...
        protected override IQueryable<User> Query()
        {
            ...
            var systemAdminUserQuery = base.Query().Where(u => u.Id == systemAdminUserId)
                .Include(u => u.UserRoles).ThenInclude(ur => ur.Role);
            var otherUsersQuery = base.Query().Where(u => u.Id != systemAdminUserId)
        ...
        public async Task<Result<UserResponse>> GetItem(LoginRequest loginRequest, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                response = user.Map<User, UserResponse>(Config);
                return Success(response);
            }
            catch (Exception exception)
            {
                LogError($"UserServiceException: {GetType().Name}.GetItem(loginRequest.UserName = {loginRequest.UserName}): {exception.Message}");
