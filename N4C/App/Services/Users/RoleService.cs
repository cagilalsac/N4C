...
namespace N4C.App.Services.Users
{
    public class RoleService : Service<Role, RoleRequest, RoleResponse>
    {
            ...
            { 
                c.Response
                    ...
                    .Select(ur => new UserResponse()
                    {
                        ...
                        ActiveS = ur.User.Active.ToHtml(c.TrueHtml, c.FalseHtml)
                    }).ToList());
                c.TitleTR = "Rol";
            });
        }
        ...
        protected override IQueryable<Role> Query()
        {
            ...
            var systemAdminRoleQuery = base.Query().Where(r => r.Id == systemAdminRoleId)
                .Include(r => r.UserRoles).ThenInclude(ur => ur.User);
            var otherRolesQuery = base.Query().Where(r => r.Id != systemAdminRoleId)
